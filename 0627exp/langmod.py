# -*- coding: utf-8 -*-
"""
Created on Wed Jun 27 08:44:22 2018
...
# #temp = pd.read_csv("C:/Users/gsoun/Downloads/Temp-repo-master/dataset.csv")
# #pd.read_csv("./part-m-00000 - Copy.csv", delimiter='|', header=None)
# #temp = temp.fillna("")
# #tempn = pd.DataFrame()
...
for i in temp.columns:
    tempn[str(i)] = '*' + temp[str(i)].map(str) + '*'
npdf = copy.deepcopy(tempn[tempn['output'] =="*0*"])
    ...
    for i in cols:
        ...
        if rejli[str(i)] !=[]:
            for j in rejli[str(i)]:
                ...
                if list(tr) == []:
                    vi = 0
                    tem = list(copy.deepcopy(rejrli[str(i)][rejli[str(i)].index(j)]))
                    tem.pop(0)
                    tem.pop(-1)
                    ...
                    for k in list(cpairs):
                        ...
                        if co == 0:
                            ...
                            vi *= v/total[str(i)]
                    if vi ==0:
                        tel = list(copy.deepcopy(rejrli[str(i)][rejli[str(i)].index(j)]))
                        tel.pop(0)
                        tel.pop(-1)
